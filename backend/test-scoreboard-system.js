/**
 * Simple test script to demonstrate the scoreboard WebSocket system
 * This script shows how the scoreboard system would work in practice
 */
...
async function testScoreboardSystem() {
  ...
  try {
    ...
    const sessionResponse = await axios.post(`${BASE_URL}/api/scoreboard/session`, {
      matchId: '507f1f77bcf86cd799439011' // Example match ID
    });
    
    const { sessionId, matchId } = sessionResponse.data.session;
    ...
    return new Promise((resolve, reject) => {
      ...
      socket.on('scoreboard:authenticated', (data) => {
        ...
        console.log(`   📊 Initial scoreboard data:`, JSON.stringify(data, null, 2));
        console.log('');
